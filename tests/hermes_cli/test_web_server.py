...
class TestWebServerEndpoints:
    ...
    def test_dashboard_v2_portfolio_uses_only_registered_ventures(self, tmp_path, monkeypatch):
        ...
        assert data["portfolio_health"]["total_projects"] == 3
    ...
    def test_dashboard_v2_source_registry_layer_exposes_registries_pipeline_and_brief(self, tmp_path, monkeypatch):
        ...
        assert resp.status_code == 200
        data = self._assert_json_response_is_not_spa_html(resp)
        assert [item["name"] for item in data["venture_registry"]["ventures"]] == ["BureauOS", "Parlay Analyzer", "TrustBase"]
        ...
        for forbidden in ["RegTech", "Engineering Brand", "Artist Management", "Research Office", "Frontend Streaming Platform"]:
    ...
    def test_dashboard_v2_summary_contract_combines_kanban_reports_and_usage(self, tmp_path, monkeypatch):
        ...
        assert empty_data["executive_briefing"]["top_priorities"] == []
        assert empty_data["executive_briefing"]["blocked_tasks"] == []
    ...
    def test_dashboard_v2_summary_returns_safe_empty_sections_when_sources_fail(self, monkeypatch):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["executive_briefing"]["top_priorities"] == []
        assert data["executive_briefing"]["blocked_tasks"] == []
        assert data["executive_briefing"]["review_required_tasks"] == []
        assert data["executive_briefing"]["completed_tasks"] == []
